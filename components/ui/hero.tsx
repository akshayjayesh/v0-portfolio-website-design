  ...
  (
    ...
    return (
      <section
        ref={ref}
        className={cn(
          "relative z-0 flex min-h-[80vh] w-full flex-col items-center justify-center overflow-visible rounded-md bg-background",
          className,
        )}
        {...props}
      >
            ...
            {actions && actions.length > 0 && (
              <div className={cn("flex gap-4", actionsClassName)}>
                {actions.map((action, index) => (
                  <Button key={index} variant={(action.variant as any) || "default"} asChild>
